...
```{r}
library(pacman)
p_load(network, statnet, sna, numDeriv)
```
...
relations
```
Why Factrs are dangerous
```{r}
#Why did we set stringsAsFactors=FALSE?
relations_wFactors<-read.csv("./data/vertexAttributes.csv",header=T,stringsAsFactors=TRUE)
relations_wFactors
```
```{r}
...
Edge / ties / links / associations

```{r}
edgelist<-read.csv("./data/edgeList.csv",header=T,stringsAsFactors=F)
head(edgelist)
```
```{r}
?network()
...
See edgeset.constructors for details and optional additional arguments
...
plot(nedge,
     displaylabels=T,
     edge.lwd=as.sociomatrix(nedge,"weight")*5)
```
...
```{r}
relations[,] # Another way to do it
```
...
```{r}
#Delete edges
g[3,5] <- 1 # Remove the edge from 3 to 5
g[3,1] <- 1
g[4,5] <- 1
as.sociomatrix(g) # Its gone!
```
...
```{r}
nrelations
plot(nrelations, displaylabels = TRUE)
summary(nrelations)
```
...
```{r}
all(m[upper.tri(m)] == m[upper.tri(m)>0])
```
...
```{r}
m<-symmetrize(m,rule="upper") #pper: Copy the upper triangle over the lower triangle
m
```
...
```{r}
nrelations %e% "strength" <- m # Add the valued ties back in - presume %e% is for the "edge" atttibutes
...
nrelations
```
Add in extra variable to the vertex attributes
...
```{r}
load("./statnet_jasny_NetworkAnalysis/IntroToSNAinR.Rdata") # Load supplemental workshop data
```


###########################################################
##4.2 Network data in SNA
#The sna package can handle network data in many forms. 

```{r}
data(flo)
flo # Adjacency form
```
For instance, the function gden() calculates network density; we can use it on a network object, an adjacency matrix, a list of such matrices, etc.
```{r}
gden(flo)
```
...
#The sna package also supports a special kind of matrix called an \sna edgelist." These are three-column matrices, each row of which represents an edge (via its sender, recipient, and value, respectively). 

These sna edgelists" have special attributes that indicate their size, vertex names (if any), and bipartite status (if applicable).

NB Bipartite graphs involve two sets with each edge going from a vertex in one set to a vertex in the other set. There are no edges connecting vertices within the same set. 
...
"graph" indicates that edges are undirected; 
"twomode" indicates that data should be interpreted as two-mode (i.e., rows and columns are distinct vertex sets). gmode is set to "digraph" by default.
...
```
```{r}
as.sociomatrix(mids_1993)
```

NB - mids_1993 lacks any information about connections - just has a en empty matrix
...
```{r}
plot(x=1:length(cd$cdist),y=(cd$cdist),xlab="Size/Distance",ylab="Frequency")
```
...
```{r}
gplot(mids_1993, 
      vertex.cex=(ideg+odeg)^0.5/2, #total degree of a node - sum of in and out degree - arbitrary scale
      vertex.sides=50,
      label.cex=0.4,
...
```{r}
gplot(mids_1993, 
      vertex.cex=1, #total degree of a node - sum of in and out degree - used to arbitrary scalethe size of the node/vertex
      vertex.sides=50,
      label.cex=0.4,
      vertex.col=rgb(odeg/max(odeg),0,ideg/max(ideg)),
      edge.lwd = 1,
...
Proportion of shortest paths between all other pairs of nodes that the given node lies on
...
```{r}
str(geodist(contig_1993))
```

Pull oit just the gdist from the generated list of counts and gdist

```{r}
contig_1993_gdist<-geodist(contig_1993)$gdist # matrix of geodesic distances
contig_1993_gdist
```
Just for row one - country one which is USA

```{r}
1/sum(contig_1993_gdist[1,2:186]) # calculate closeness for country 1
```

The above sums the entries and then 1 / 

Problem that some entries have INF

```{r}
which(contig_1993_gdist[1,]=="Inf") # disconnected matrix
```

Here we 1 / each entry and then sum it = where 1 / Inf = 0

```{r}
sum(1/contig_1993_gdist[1,2:186]) # alternate closeness for country 1; meed to ignore the 0 distance for self 1,1
```
...
sum across row one's columns

```{r}
infEntries.vec <- which(contig_1993_gdist[,1]=="Inf") #  matrix of unnconnected
infEntries.vec
...
```{r}
filter <- 1:186 %not_in% infEntries.vec
filter
```

```{r}
nonInf.vec <- contig_1993_gdist[1,filter]
nonInf.vec
```
...
```{r}
nonInf.vec <- nonInf.vec[nonInf.vec %not_in% 0]
nonInf.vec
```
...
closeness2 <- function(x){ # Create an alternate closeness function!
  ...
  apply(geo, 1, sum) # Return sum(1/geodist) for each vertex, 1 = by row, sum each row
}
```
